...
export function ContactForm() {
  ...
  const formulesOptions = useMemo(
    () => [
      { value: "", label: t("planPlaceholder") },
      ...services.map((s) => ({ value: s.nom, label: `${s.nom} · ${s.prixBase} €` })),
      { value: "autre", label: t("planUnknown") },
    ],
    [t]
  );
        ...
        <div className="rounded-2xl border border-white/10 bg-white/3 p-5 space-y-4 mb-2">
          ...
          {selectedAddonObjects.length > 0 ? (
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-white/50 mb-2">
                {t("selectedOptions", { count: selectedAddonObjects.length })}
              </p>
            ...
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-white/50 mb-2">
                {t("includedInPlan", { count: service.inclus.length })}
              </p>
          ...
          <div className="border-t border-white/6 pt-3">
            <p className="text-xs text-white/65 leading-relaxed">
              <span className="text-white/85 font-medium">{t("adjustSelectionBold")}</span>{" "}
              {t("adjustSelectionNote")}
            </p>
          </div>
        </div>
      )}
