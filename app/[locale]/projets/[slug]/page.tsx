...
export async function generateStaticParams() {
  return routing.locales.flatMap((locale) =>
    projets.map((p) => ({ locale, slug: p.slug }))
  );
}
...
function TechSection({ challenges, label }: { challenges: Challenge[]; label: string }) {
  if (!challenges.length) return null;
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <div className="w-7 h-[3px] rounded-full shrink-0" style={{ background: "var(--gold)" }} />
        <p className="text-[10px] font-bold uppercase tracking-[0.3em]" style={{ color: "var(--gold)" }}>{label}</p>
      </div>
...
export default async function ProjetPage({ params }: Props) {
  ...
  // Project text fields — from translations when available, fallback to lib/projets.ts data
  const projetTitre = tProjet.has(`${slug}.titre`) ? tProjet(`${slug}.titre`) : projet.titre;
  const projetIntro = tProjet.has(`${slug}.intro`) ? tProjet(`${slug}.intro`) : projet.intro;
  const projetTagline = projet.tagline && tProjet.has(`${slug}.tagline`) ? tProjet(`${slug}.tagline`) : projet.tagline;
  const projetDescPublic = projet.descriptionPublic && tProjet.has(`${slug}.descriptionPublic`) ? tProjet(`${slug}.descriptionPublic`) : projet.descriptionPublic;
  const projetDescTech = projet.descriptionTech && tProjet.has(`${slug}.descriptionTech`) ? tProjet(`${slug}.descriptionTech`) : projet.descriptionTech;
  const projetResultats = projet.resultats && tProjet.has(`${slug}.resultats`) ? tProjet(`${slug}.resultats`) : projet.resultats;

  type RawChallenge = { titre: string; solution: string };
  type RawSliderSet = { title: string; description: string; images: string[] };
  type RawTechnology = { nom: string; detail: string };
  type RawCredit = { nom: string; role: string };
  type RawPointsCle = { label: string; items: string[] };
  type RawPillarCard = { eyebrow: string; titre: string; description: string };

  const projetChallenges = tProjet.has(`${slug}.challenges`)
    ? (tProjet.raw(`${slug}.challenges`) as RawChallenge[])
    : projet.challenges;
  const projetSliderSets = tProjet.has(`${slug}.sliderSets`)
    ? (tProjet.raw(`${slug}.sliderSets`) as RawSliderSet[]).map((s, i) => ({
        ...s,
        images: projet.sliderSets?.[i]?.images ?? [],
      }))
    : projet.sliderSets;
  const projetTechnologies = tProjet.has(`${slug}.technologies`)
    ? (tProjet.raw(`${slug}.technologies`) as RawTechnology[])
    : projet.technologies;
  const projetCredits = tProjet.has(`${slug}.credits`)
    ? (tProjet.raw(`${slug}.credits`) as RawCredit[])
    : projet.credits;
  const projetImageCaptions = tProjet.has(`${slug}.imageCaptions`)
    ? (tProjet.raw(`${slug}.imageCaptions`) as string[])
    : projet.imageCaptions;
  const projetPointsCles = tProjet.has(`${slug}.pointsCles`)
    ? (tProjet.raw(`${slug}.pointsCles`) as RawPointsCle[])
    : projet.pointsCles;
  const projetPillarsCards = projet.pillarsCards && tProjet.has(`${slug}.pillarsCards`)
    ? (tProjet.raw(`${slug}.pillarsCards`) as RawPillarCard[]).map((p, i) => ({
        ...p,
        iconName: projet.pillarsCards![i]?.iconName,
      }))
    : projet.pillarsCards;
  ...
  return (
    <div className="pt-28 pb-28">
        ...
        <div className="grid lg:grid-cols-[1fr_300px] gap-12 items-start">
          ...
          <div className="space-y-3 lg:pt-2 hidden sm:block">
            <div className="p-5 rounded-xl border border-border space-y-4" style={{ background: "linear-gradient(135deg, hsl(163 52% 76% / 0.05) 0%, hsl(var(--card)) 60%)" }}>
              <div>
                <p className="text-[10px] uppercase tracking-[0.25em] mb-2 font-semibold" style={{ color: "var(--accent)" }}>{t("meta.client")}</p>
              ...
              <div>
                <p className="text-[10px] uppercase tracking-[0.25em] mb-1.5 font-semibold" style={{ color: "var(--accent)" }}>{t("meta.context")}</p>
                <div className="flex items-center gap-1.5 text-sm text-foreground/88">
                  {projet.contexte === "agence" ? (
                    <><Briefcase size={11} className="text-(--lavender)" /><span>{t("context.agence")}</span></>
              ...
              <div>
                <p className="text-[10px] uppercase tracking-[0.25em] mb-1.5 font-semibold" style={{ color: "var(--accent)" }}>{t("meta.year")}</p>
                <p className="text-sm font-semibold text-foreground/85">{year}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-[0.25em] mb-2 font-semibold" style={{ color: "var(--accent)" }}>{t("meta.stack")}</p>
                <div className="flex flex-wrap gap-1.5">
                  {projet.tags.map((tag, ti) => (
                    <span
                      key={tag}
      ...
      <div className="max-w-4xl mx-auto px-6 lg:px-10 space-y-16 sm:space-y-24">
        ...
        {(projet.video || projet.img || projetDescPublic) && (
              ...
              <>
                ...
                {projetDescPublic && (
                  <div className="max-w-2xl pt-2">
                    {renderParagraphs(projetDescPublic)}
                  </div>
                )}
              </>
            ) : (
                  ...
                  <div className="mx-auto md:mx-0">
                    {projet.video ? (
                      ...
                      <div className="relative rounded-xl overflow-hidden border border-border/60 shadow-[0_32px_80px_-8px_rgba(0,0,0,0.6)]">
                        <Image src={projet.img!} alt={projetTitre} width={400} height={800} className="w-auto max-h-[520px] object-contain" priority />
                      </div>
                    )}
                  </div>
                )}
        ...
        {projetPillarsCards && projetPillarsCards.length > 0 ? (
          <PillarsSection pillars={projetPillarsCards as NonNullable<typeof projet.pillarsCards>} />
        ) : projetPointsCles && projetPointsCles.length > 0 && (
          <div className="flex flex-col items-center">
            <p className="text-sm font-bold uppercase tracking-wider text-foreground mb-5">
              {projetPointsCles.map((g) => g.label).join(" · ")}
            </p>
            <ul className="flex flex-col gap-2.5">
              {projetPointsCles.flatMap((group) => group.items).map((item, i) => (
        ...
        {projet.wideMedia && zigzagImages.length > 0 ? (
          ...
          <div className="flex flex-col-reverse md:grid md:grid-cols-[1fr_auto] gap-8 md:gap-10 items-center">
            <div>
              {(projetImageCaptions?.[0] ?? projetDescPublic) && renderParagraphs(projetImageCaptions?.[0] ?? projetDescPublic ?? "")}
            </div>
            <ProjetImageSlider
        ...
        {interleaveZigzag && projetChallenges && (
          <TechSection challenges={projetChallenges.slice(0, 2)} label={t("section.technical")} />
        )}
        ...
        {!projet.wideMedia && zigzagImages.length >= 3 && (() => {
          ...
          return (
            <div className="grid md:grid-cols-[auto_1fr] gap-10 items-center">
              ...
              />
              <div>
                {(projetImageCaptions?.[1] ?? projetDescTech) && renderParagraphs(projetImageCaptions?.[1] ?? projetDescTech ?? "")}
              </div>
            </div>
          );
        })()}
        ...
        {interleaveZigzag && projetChallenges && projetChallenges.length > 2 && (
          <TechSection challenges={projetChallenges.slice(2)} label={t("section.technical")} />
        )}
        ...
        {projetSliderSets && projetChallenges && projetChallenges.length > 0 ? (() => {
          ...
          const sectionOffset = (projet.video || projetTagline) ? 1 : 0;
          const batch1 = projetChallenges.slice(0, splitAt);
          const batch2 = projetChallenges.slice(splitAt);
          ...
          const SliderSetBlock = ({ set, si }: { set: { title: string; description: string; images: string[] }; si: number }) => (
            <div className="space-y-5">
              ...
              {projet.customSlider === "bald-identity" && set.images.length === 0
                ...
                : <ProjetImageSlider images={set.images} alt={`${projetTitre} — ${set.title}`} wide={!!projet.wideMedia} aspectRatio={wideAspectRatio} />
              }
            </div>
          );
          ...
          return (
            <>
              {batch1.length > 0 && <TechSection challenges={batch1} label={t("section.technical")} />}
              <SliderSetBlock set={projetSliderSets![0]} si={0} />
              {batch2.length > 0 && <TechSection challenges={batch2} label={t("section.technical")} />}
              {projetSliderSets!.slice(1).map((set, i) => (
          ...
          <>
            {projetSliderSets && projetSliderSets.map((set, si) => (
              <div key={si} className="space-y-5">
                <div className="relative overflow-hidden pb-1">
                  <span
                    ...
                    aria-hidden="true"
                  >
                    {String(si + 1 + ((projet.video || projetTagline) ? 1 : 0)).padStart(2, "0")}
                  </span>
                  <p className="text-[10px] font-semibold uppercase tracking-[0.25em] mb-2 relative z-10" style={{ color: "var(--accent)" }}>
                    {String(si + 1 + ((projet.video || projetTagline) ? 1 : 0)).padStart(2, "0")} —
                  </p>
            ...
            {!interleaveZigzag && projetChallenges && projetChallenges.length > 0 && (
              <TechSection challenges={projetChallenges} label={t("section.technical")} />
            )}
          </>
        )}
        ...
        {/* TECHNOLOGIES */}
        {projetTechnologies && projetTechnologies.length > 0 && (
          <div>
            ...
            <div className="space-y-3">
              {projetTechnologies.map((tech, ti) => {
        ...
        {projetResultats && zigzagImages.length < 3 && (
          <div
          ...
          >
            ...
            <p className="text-sm text-foreground/88 leading-relaxed">{projetResultats}</p>
          </div>
        )}

        {/* CRÉDITS */}
        {projetCredits && projetCredits.length > 0 && (
          <div>
            <div className="flex items-center gap-4 mb-6">
              <div className="w-7 h-[3px] rounded-full shrink-0" style={{ background: "var(--mauve)" }} />
              <h2 className="text-[10px] font-bold uppercase tracking-[0.3em]" style={{ color: "var(--mauve)" }}>{t("section.team")}</h2>
            </div>
            <div className="flex flex-wrap gap-3">
              {projetCredits.map((c, i) => {
