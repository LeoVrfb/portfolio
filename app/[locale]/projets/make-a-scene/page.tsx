...
function renderHighlight(text: string): React.ReactNode {
  ...
  return parts.map((part, i) => {
    ...
    if (/^\*[^*]+\*$/.test(part))
      return <em key={i} className="font-serif-display italic font-normal" style={{ color: "var(--mauve)" }}>{part.slice(1, -1)}</em>;
...
export default async function MakeASceneePage({ params }: Props) {
  const { locale } = await params;
  ...
  const sliderSets = (tProjet.raw(`${SLUG}.sliderSets`) as { title: string; description: string }[]).map(
    (set, i) => ({ ...set, images: projet.sliderSets?.[i]?.images ?? [] })
  );
  ...
  return (
    <div className="pt-28 pb-28">
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-10 mb-12">
        ...
        <div className="grid lg:grid-cols-[1fr_320px] gap-12 items-start">
          <Reveal duration={0.55} y={18}>
            ...
            <p className="text-sm uppercase tracking-widest mb-5" style={{ color: "var(--mauve)" }}>{projetTitre}</p>
          ...
          <Reveal duration={0.55} y={18} delay={0.12} className="space-y-3 lg:pt-2 hidden sm:block">
            <div className="p-5 rounded-xl border border-border space-y-4"
              ...
              <div>
                <p className="text-[10px] uppercase tracking-[0.25em] mb-2 font-semibold" style={{ color: "var(--mauve)" }}>{t("meta.client")}</p>
                <p className="text-sm font-semibold text-foreground">{projet.client}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-[0.25em] mb-1.5 font-semibold" style={{ color: "var(--mauve)" }}>{t("meta.context")}</p>
                <div className="flex items-center gap-1.5 text-sm text-foreground/88">
                  <Briefcase size={11} className="text-(--mauve)" />
                  <span>{t("context.agenceInterne")}</span>
                </div>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-[0.25em] mb-1.5 font-semibold" style={{ color: "var(--mauve)" }}>{t("meta.year")}</p>
                <p className="text-sm font-semibold text-foreground/85">{year}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-[0.25em] mb-2 font-semibold" style={{ color: "var(--mauve)" }}>{t("meta.stack")}</p>
                <div className="flex flex-wrap gap-1.5">
                  {projet.tags.map((tag, ti) => (
                    <span key={tag} className="text-[10px] font-medium px-2 py-0.5 rounded-full border"
                      style={{ color: TAG_COLORS[ti % TAG_COLORS.length].color, background: TAG_COLORS[ti % TAG_COLORS.length].bg, borderColor: TAG_COLORS[ti % TAG_COLORS.length].color, opacity: 0.9 }}>
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-10 space-y-24 sm:space-y-32">
        ...
        <div className="h-px" style={{ background: "linear-gradient(90deg, transparent, hsl(290 16% 58% / 0.3) 40%, hsl(284 24% 70% / 0.2) 60%, transparent)" }} />
          ...
          <Reveal>
            <header className="grid grid-cols-1 lg:grid-cols-[180px_1fr] gap-4 lg:gap-12 items-baseline">
              <div className="flex items-center gap-3 lg:block lg:pt-1">
                <span className="text-[10px] font-bold uppercase tracking-[0.3em]" style={{ color: "var(--mauve)" }}>{t("section.overview")}</span>
                <span className="font-black leading-none lg:block lg:mt-2" style={{ fontSize: "clamp(2.5rem, 4.5vw, 4rem)", color: "var(--mauve)", opacity: 0.95 }}>01</span>
              </div>
        ...
        <section className="space-y-10">
          <Reveal>
            <header className="grid grid-cols-1 lg:grid-cols-[180px_1fr] gap-4 lg:gap-12 items-baseline">
              <div className="flex items-center gap-3 lg:block lg:pt-1">
                <span className="text-[10px] font-bold uppercase tracking-[0.3em]" style={{ color: "var(--mauve)" }}>{t("section.stack")}</span>
        ...
        <Reveal>
          <div className="relative rounded-2xl overflow-hidden px-7 py-8 flex flex-col sm:flex-row sm:items-center gap-5 sm:gap-8"
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[10px] uppercase tracking-[0.3em] font-semibold mb-2" style={{ color: "var(--mauve)" }}>
      ...
      <Link href={`/projets/${prevProjet.slug}`}
        ...
        <div className="text-left">
          <p className="text-[9px] uppercase tracking-[0.25em] font-semibold mb-0.5" style={{ color: "var(--mauve)" }}>{t("prevProject")}</p>
      ...
      <Link href={`/projets/${nextProjet.slug}`}
        ...
        <div className="text-right">
          <p className="text-[9px] uppercase tracking-[0.25em] font-semibold mb-0.5" style={{ color: "var(--mauve)" }}>{t("nextProject")}</p>
