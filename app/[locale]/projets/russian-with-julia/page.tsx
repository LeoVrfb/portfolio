...
function renderHighlight(text: string): React.ReactNode {
  ...
  return parts.map((part, i) => {
    ...
    if (/^\*[^*]+\*$/.test(part))
      return <em key={i} className="font-serif-display italic font-normal" style={{ color: "var(--accent)" }}>{part.slice(1, -1)}</em>;
...
export default async function RussianWithJuliaPage({ params }: Props) {
  const { locale } = await params;
  ...
  const rawPillars = tProjet.raw(`${SLUG}.pillarsCards`) as { eyebrow: string; titre: string; description: string }[];
  const pillars = rawPillars.map((p, i) => ({
    ...p,
    iconName: projet.pillarsCards?.[i]?.iconName ?? "sparkles" as PillarIcon,
  }));
    ...
    <div className="pt-28 pb-28">
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-10 mb-12">
        ...
        <div className="grid lg:grid-cols-[1fr_320px] gap-12 items-start">
          <Reveal duration={0.55} y={18}>
            ...
            <p className="text-sm uppercase tracking-widest mb-5" style={{ color: "var(--accent)" }}>{projetTitre}</p>
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-10 space-y-24 sm:space-y-32">
          ...
          <Reveal>
            <header className="grid grid-cols-1 lg:grid-cols-[180px_1fr] gap-4 lg:gap-12 items-baseline">
              <div className="flex items-center gap-3 lg:block lg:pt-1">
                <span className="text-[10px] font-bold uppercase tracking-[0.3em]" style={{ color: "var(--accent)" }}>{t("section.overview")}</span>
                <span className="font-black leading-none lg:block lg:mt-2" style={{ fontSize: "clamp(2.5rem, 4.5vw, 4rem)", color: "var(--accent)", opacity: 0.95 }}>01</span>
              </div>
        ...
        <section className="space-y-10 lg:space-y-12">
          <Reveal>
            <header className="grid grid-cols-1 lg:grid-cols-[180px_1fr] gap-4 lg:gap-12 items-baseline">
              <div className="flex items-center gap-3 lg:block lg:pt-1">
                <span className="text-[10px] font-bold uppercase tracking-[0.3em]" style={{ color: "var(--mauve)" }}>
                  {tProjet(`${SLUG}.page.section02.sectionLabel`)}
                </span>
                <span className="font-black leading-none lg:block lg:mt-2" style={{ fontSize: "clamp(2.5rem, 4.5vw, 4rem)", color: "var(--mauve)", opacity: 0.95 }}>02</span>
              </div>
              <div className="max-w-2xl">
                ...
                <p className="mt-5 text-base text-foreground/72 leading-relaxed">
                  {tProjet(`${SLUG}.page.section02.subtitle`)}
                </p>
              </div>
            </header>
          </Reveal>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-6">
            {pillars.map((p, i) => {
              const Icon = PILLAR_ICONS[p.iconName as PillarIcon];
              const c = PILLAR_COLORS[i % PILLAR_COLORS.length];
              return (
                <Reveal key={i} delay={0.06 + i * 0.08}>
                  <div className="relative rounded-2xl overflow-hidden flex flex-col gap-5 p-7"
                    ...
                    <span className="absolute top-4 right-5 font-black leading-none select-none pointer-events-none"
                      style={{ fontSize: "3.5rem", color: c.tint, opacity: 0.12 }} aria-hidden>
                    ...
                    <p className="text-[9px] font-bold uppercase tracking-[0.35em]" style={{ color: c.tint }}>{p.eyebrow}</p>
          ...
          return (
            <section key={si} className="space-y-8">
              <Reveal>
                <header className="grid grid-cols-1 lg:grid-cols-[180px_1fr] gap-4 lg:gap-12 items-baseline">
                  <div className="flex items-center gap-3 lg:block lg:pt-1">
                    ...
                    <span className="font-black leading-none lg:block lg:mt-2" style={{ fontSize: "clamp(2.5rem, 4.5vw, 4rem)", color: "var(--accent)", opacity: 0.95 }}>{sectionNum}</span>
                  </div>
                  <div className="max-w-2xl">
              ...
              <Reveal delay={0.08} y={20} duration={0.6}>
                <ProjetImageSlider images={set.images} alt={`${projetTitre} — ${set.title}`} wide aspectRatio="aspect-[2/1]" />
              </Reveal>
            </section>
          );
        })}
            ...
            <Reveal>
              <header className="grid grid-cols-1 lg:grid-cols-[180px_1fr] gap-4 lg:gap-12 items-baseline">
                <div className="flex items-center gap-3 lg:block lg:pt-1">
                  ...
                  <span className="font-black leading-none lg:block lg:mt-2" style={{ fontSize: "clamp(2.5rem, 4.5vw, 4rem)", color: "var(--gold)", opacity: 0.95 }}>06</span>
                </div>
                <div className="max-w-2xl">
                  <h2 className="text-2xl sm:text-3xl lg:text-4xl font-black tracking-[-0.02em] text-foreground leading-[1.1]">
          ...
          <Reveal>
            <header className="grid grid-cols-1 lg:grid-cols-[180px_1fr] gap-4 lg:gap-12 items-baseline">
              <div className="flex items-center gap-3 lg:block lg:pt-1">
                ...
                <span className="font-black leading-none lg:block lg:mt-2" style={{ fontSize: "clamp(2.5rem, 4.5vw, 4rem)", color: "var(--lavender)", opacity: 0.95 }}>07</span>
              </div>
              <div className="max-w-2xl">
                <h2 className="text-2xl sm:text-3xl lg:text-4xl font-black tracking-[-0.02em] text-foreground leading-[1.1]">
                  {tProjet(`${SLUG}.page.section07.titlePart1`)}
                  {" "}
                  <em className="font-serif-display italic font-normal" style={{ color: "var(--lavender)" }}>
                    {tProjet(`${SLUG}.page.section07.titlePart2`)}
                  </em>
                  {" "}
                  {tProjet(`${SLUG}.page.section07.titlePart3`)}
                </h2>
              </div>
            </header>
          </Reveal>
        ...
        <Reveal delay={0.05}>
          <Link href={`/projets/${nextProjet.slug}`} className="flex items-center justify-between p-5 rounded-xl border border-border hover:border-white/15 transition-colors cursor-pointer group">
            <div>
              <p className="text-[10px] uppercase tracking-[0.3em] mb-1.5 font-semibold" style={{ color: "var(--accent)" }}>{t("nextProject")}</p>
              <p className="text-base font-bold text-foreground group-hover:text-accent transition-colors">{nextProjet.clientShort ?? nextProjet.client}</p>
      ...
      <Link href={`/projets/${prevProjet.slug}`}
        ...
        <div className="text-left">
          <p className="text-[9px] uppercase tracking-[0.25em] font-semibold mb-0.5" style={{ color: "var(--accent)" }}>{t("prevProject")}</p>
      ...
      <Link href={`/projets/${nextProjet.slug}`}
        ...
        <div className="text-right">
          <p className="text-[9px] uppercase tracking-[0.25em] font-semibold mb-0.5" style={{ color: "var(--accent)" }}>{t("nextProject")}</p>
