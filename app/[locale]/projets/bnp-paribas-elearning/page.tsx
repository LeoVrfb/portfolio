/* ──────────────────────────────────────────────────────────────────────────
   PAGE PROJET — BNP Paribas (e-learning IA générative)
   ────────────────────────────────────────────────────────────────────────── */
...
const SLUG = "bnp-paribas-elearning";
...
function renderHighlight(text: string): React.ReactNode {
  ...
  return parts.map((part, i) => {
    ...
    if (/^\*[^*]+\*$/.test(part))
      return <em key={i} className="font-serif-display italic font-normal" style={{ color: "var(--lavender)" }}>{part.slice(1, -1)}</em>;
    ...
    if (linkMatch) {
      ...
      return (
        <a key={i} href={href} {...(external ? { target: "_blank", rel: "noopener noreferrer" } : {})}
          ...
          style={{ color: "var(--lavender)", textDecorationColor: "color-mix(in oklab, var(--lavender) 50%, transparent)" }}>
          {label}
        </a>
      );
    }
...
export default async function BnpParibasPage({ params }: Props) {
  const { locale } = await params;
  ...
  const sliderSets = (tProjet.raw(`${SLUG}.sliderSets`) as { title: string; description: string; images: string[] }[])
  ...
  return (
    <div className="pt-28 pb-28">
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-10 mb-12">
        ...
        <div className="grid lg:grid-cols-[1fr_320px] gap-12 items-start">
          <Reveal duration={0.55} y={18}>
            ...
            <p className="text-sm uppercase tracking-widest mb-5" style={{ color: "var(--lavender)" }}>
              {projetTitre}
            </p>
          ...
          <Reveal duration={0.55} y={18} delay={0.12} className="space-y-3 lg:pt-2 hidden sm:block">
            <div className="p-5 rounded-xl border border-border space-y-4"
              style={{ background: "linear-gradient(135deg, hsl(284 24% 70% / 0.05) 0%, hsl(var(--card)) 60%)" }}>
              <div>
                <p className="text-[10px] uppercase tracking-[0.25em] mb-2 font-semibold" style={{ color: "var(--lavender)" }}>{t("meta.client")}</p>
                <p className="text-sm font-semibold text-foreground">{projet.client}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-[0.25em] mb-1.5 font-semibold" style={{ color: "var(--lavender)" }}>{t("meta.context")}</p>
              ...
              <div>
                <p className="text-[10px] uppercase tracking-[0.25em] mb-1.5 font-semibold" style={{ color: "var(--lavender)" }}>{t("meta.year")}</p>
                <p className="text-sm font-semibold text-foreground/85">{year}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-[0.25em] mb-2 font-semibold" style={{ color: "var(--lavender)" }}>{t("meta.stack")}</p>
                <div className="flex flex-wrap gap-1.5">
                  {projet.tags.map((tag, ti) => (
                    <span key={tag} className="text-[10px] font-medium px-2 py-0.5 rounded-full border"
                      style={{ color: TAG_COLORS[ti % TAG_COLORS.length].color, background: TAG_COLORS[ti % TAG_COLORS.length].bg, borderColor: TAG_COLORS[ti % TAG_COLORS.length].color, opacity: 0.9 }}>
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-10 space-y-24 sm:space-y-32">
        ...
        <div className="h-px" style={{ background: "linear-gradient(90deg, transparent, hsl(284 24% 70% / 0.3) 40%, hsl(163 52% 76% / 0.2) 60%, transparent)" }} />
          ...
          <Reveal>
            <header className="grid grid-cols-1 lg:grid-cols-[180px_1fr] gap-4 lg:gap-12 items-baseline">
              <div className="flex items-center gap-3 lg:block lg:pt-1">
                <span className="text-[10px] font-bold uppercase tracking-[0.3em]" style={{ color: "var(--lavender)" }}>
                  {t("section.overview")}
                </span>
                <span className="font-black leading-none lg:block lg:mt-2" style={{ fontSize: "clamp(2.5rem, 4.5vw, 4rem)", color: "var(--lavender)", opacity: 0.95 }}>01</span>
              </div>
              <div className="max-w-3xl">
                <h2 className="text-2xl sm:text-3xl lg:text-4xl font-black tracking-[-0.02em] text-foreground leading-[1.1]">
                  ...
                  {" "}
                  {tProjet(`${SLUG}.page.section01.titlePart3`)}
                </h2>
            ...
            <Reveal delay={0.08} y={20} duration={0.65}>
              <figure className="space-y-3">
                <div className="relative w-full aspect-video rounded-xl overflow-hidden border border-border/60 shadow-[0_32px_80px_-8px_rgba(0,0,0,0.6)]">
                  <WideVideoPlayer src={projet.video} loopStart={projet.videoLoopStart} loopEnd={projet.videoLoopEnd} className="absolute inset-0 w-full h-full object-cover" />
                </div>
        ...
        {sliderSets.map((set, si) => {
          ...
          const accent = si % 2 === 0 ? "var(--accent)" : "var(--gold)";
          return (
            <section key={si} className="space-y-8">
              <Reveal>
                <header className="grid grid-cols-1 lg:grid-cols-[180px_1fr] gap-4 lg:gap-12 items-baseline">
                  <div className="flex items-center gap-3 lg:block lg:pt-1">
                    <span className="text-[10px] font-bold uppercase tracking-[0.3em]" style={{ color: accent }}>{t("section.section")}</span>
                    <span className="font-black leading-none lg:block lg:mt-2" style={{ fontSize: "clamp(2.5rem, 4.5vw, 4rem)", color: accent, opacity: 0.95 }}>{sectionNum}</span>
                  </div>
                  <div className="max-w-2xl">
                    <h3 className="text-2xl sm:text-3xl lg:text-4xl font-black tracking-[-0.01em] text-foreground leading-[1.1]">{set.title}</h3>
                    <p className="mt-4 text-base text-foreground/85 leading-relaxed">{renderHighlight(set.description)}</p>
                  </div>
                </header>
              </Reveal>
              <Reveal delay={0.08} y={20} duration={0.6}>
                <ProjetImageSlider images={set.images} alt={`${projetTitre} — ${set.title}`} wide aspectRatio="aspect-video" />
              </Reveal>
            </section>
          );
        })}
        ...
        <section className="space-y-10">
          <Reveal>
            <header className="grid grid-cols-1 lg:grid-cols-[180px_1fr] gap-4 lg:gap-12 items-baseline">
              <div className="flex items-center gap-3 lg:block lg:pt-1">
                <span className="text-[10px] font-bold uppercase tracking-[0.3em]" style={{ color: "var(--accent)" }}>{t("section.stack")}</span>
                <span className="font-black leading-none lg:block lg:mt-2" style={{ fontSize: "clamp(2.5rem, 4.5vw, 4rem)", color: "var(--accent)", opacity: 0.95 }}>05</span>
              </div>
              <div className="max-w-2xl">
                <h2 className="text-2xl sm:text-3xl lg:text-4xl font-black tracking-[-0.02em] text-foreground leading-[1.1]">
            ...
            <Reveal>
              <header className="grid grid-cols-1 lg:grid-cols-[180px_1fr] gap-4 lg:gap-12 items-baseline">
                <div className="flex items-center gap-3 lg:block lg:pt-1">
                  ...
                  <span className="font-black leading-none lg:block lg:mt-2" style={{ fontSize: "clamp(2.5rem, 4.5vw, 4rem)", color: "var(--mauve)", opacity: 0.95 }}>06</span>
                </div>
                <div className="max-w-2xl">
                  <h2 className="text-2xl sm:text-3xl lg:text-4xl font-black tracking-[-0.02em] text-foreground leading-[1.1]">
                  ...
                  <p className="mt-5 text-base text-foreground/72 leading-relaxed">
                    {tProjet(`${SLUG}.page.section06.subtitle`)}
                  </p>
                </div>
              </header>
            </Reveal>
        ...
        <Reveal>
          <div className="relative rounded-2xl overflow-hidden px-7 py-8 flex flex-col sm:flex-row sm:items-center gap-5 sm:gap-8"
            style={{ background: "hsl(284 24% 70% / 0.05)", border: "1px solid hsl(284 24% 70% / 0.2)" }}>
            <div className="absolute top-0 left-0 right-0 h-[3px]" style={{ background: "var(--lavender)" }} />
            <div className="flex-1 min-w-0">
              <p className="text-[10px] uppercase tracking-[0.3em] font-semibold mb-2" style={{ color: "var(--lavender)" }}>
            ...
            <Link href="/services" className="shrink-0 inline-flex items-center gap-2 px-5 py-3 rounded-xl font-semibold text-sm transition-all cursor-pointer"
              style={{ background: "var(--lavender)", color: "hsl(158 24% 7%)" }}>
              {t("viewServices")}
        ...
        <Reveal delay={0.05}>
          <Link href={`/projets/${nextProjet.slug}`} className="flex items-center justify-between p-5 rounded-xl border border-border hover:border-white/15 transition-colors cursor-pointer group">
            <div>
              <p className="text-[10px] uppercase tracking-[0.3em] mb-1.5 font-semibold" style={{ color: "var(--lavender)" }}>{t("nextProject")}</p>
              <p className="text-base font-bold text-foreground group-hover:text-(--lavender) transition-colors">{nextProjet.clientShort ?? nextProjet.client}</p>
              <p className="text-xs text-foreground/40 mt-0.5">{nextProjet.titre}</p>
            </div>
            <ArrowRight className="w-5 h-5 text-foreground/25 group-hover:text-(--lavender) group-hover:translate-x-1 transition-all shrink-0 ml-4" />
          </Link>
        </Reveal>
      </div>
      ...
      <Link href={`/projets/${prevProjet.slug}`}
        ...
        <ArrowLeft className="w-4 h-4 text-foreground/30 group-hover:text-(--lavender) group-hover:-translate-x-0.5 transition-all shrink-0" />
        <div className="text-left">
          <p className="text-[9px] uppercase tracking-[0.25em] font-semibold mb-0.5" style={{ color: "var(--lavender)" }}>{t("prevProject")}</p>
      ...
      <Link href={`/projets/${nextProjet.slug}`}
        ...
        <div className="text-right">
          <p className="text-[9px] uppercase tracking-[0.25em] font-semibold mb-0.5" style={{ color: "var(--lavender)" }}>{t("nextProject")}</p>
        ...
        <ArrowRight className="w-4 h-4 text-foreground/30 group-hover:text-(--lavender) group-hover:translate-x-0.5 transition-all shrink-0" />
      </Link>
    </div>
  );
}
