...
function renderHighlight(text: string): React.ReactNode {
  ...
  return parts.map((part, i) => {
    ...
    if (/^\*[^*]+\*$/.test(part))
      return (
        <em key={i} className="font-serif-display italic font-normal" style={{ color: "var(--accent)" }}>
          {part.slice(1, -1)}
        </em>
      );
...
export default async function ArgedisTotalEnergiesPage({ params }: Props) {
  const { locale } = await params;
  ...
  const captions = (tProjet.raw(`${SLUG}.imageCaptions`) as string[]) ?? [];
  ...
  const inNumbers = tProjet.raw(`${SLUG}.page.inNumbers`) as {
    label: string;
    stats: { value: string; label: string }[];
  };
  const interactions = tProjet.raw(`${SLUG}.page.interactions`) as {
    label: string;
  ...
  return (
    <div className="pt-28 pb-28">
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-10 space-y-24 sm:space-y-32">
        ...
        <div className="h-px" style={{ background: "linear-gradient(90deg, transparent, hsl(163 24% 54% / 0.3) 40%, hsl(47 72% 73% / 0.2) 60%, transparent)" }} />
          ...
          <Reveal>
            <header className="grid grid-cols-1 lg:grid-cols-[180px_1fr] gap-4 lg:gap-12 items-baseline">
              <div className="flex items-center gap-3 lg:block lg:pt-1">
                <span className="text-[10px] font-bold uppercase tracking-[0.3em]" style={{ color: "var(--accent)" }}>
                  {t("section.overview")}
                </span>
                <span className="font-black leading-none lg:block lg:mt-2" style={{ fontSize: "clamp(2.5rem, 4.5vw, 4rem)", color: "var(--accent)", opacity: 0.95 }}>
                  01
                </span>
              </div>
            ...
            <Reveal delay={0.08} y={20} duration={0.65}>
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-start">
                ...
                <div className="lg:col-span-7 lg:pt-2 space-y-10">
                  {descriptionPublic && (
                    <div>{renderParagraphs(descriptionPublic)}</div>
                  )}
                  ...
                  <div className="pt-8 border-t border-foreground/10">
                    ...
                    <div className="grid grid-cols-2 gap-x-8 gap-y-7">
                      {inNumbers.stats.map((stat, i) => {
                        const colors = ["var(--accent)", "var(--gold)", "var(--lavender)", "var(--mauve)"];
                        const color = colors[i % colors.length];
                        return (
                          <div key={i} className="border-l-2 pl-4" style={{ borderColor: color }}>
                            <p className="font-black leading-none mb-2" style={{ fontSize: "clamp(1.5rem, 2.4vw, 2.1rem)", color }}>
                              {stat.value}
                            </p>
            ...
            <Reveal>
              <header className="grid grid-cols-1 lg:grid-cols-[180px_1fr] gap-4 lg:gap-12 items-baseline">
                <div className="flex items-center gap-3 lg:block lg:pt-1">
                  ...
                  <span className="font-black leading-none lg:block lg:mt-2" style={{ fontSize: "clamp(2.5rem, 4.5vw, 4rem)", color: "var(--gold)", opacity: 0.95 }}>
                    02
                  </span>
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-start">
                <figure className="lg:col-span-5 mx-auto lg:mx-0 w-full max-w-[400px] lg:max-w-none lg:w-full">
                  <div className="relative rounded-2xl overflow-hidden border border-border/60 shadow-[0_32px_80px_-8px_rgba(0,0,0,0.6)]">
                    <Image
                      src={cardImage}
                      alt={`${projetTitre} — ${tProjet(`${SLUG}.page.figCaption.regional`)}`}
                      width={1080}
                      height={1920}
                  ...
                  <div className="pt-8 border-t border-foreground/10">
                    ...
                    <div className="flex flex-wrap gap-x-2.5 gap-y-2.5">
                      {[
                        ...
                        { name: tProjet(`${SLUG}.page.regions.more`), color: "hsl(0 0% 60%)" },
                      ].map((r) => (
                        <span
                          key={r.name}
        ...
        {/* 03 — DE PRODUCTEUR EN PRODUCTEUR */}
        {captions[1] && (
          <section className="space-y-10 lg:space-y-12">
            <Reveal>
              <header className="grid grid-cols-1 lg:grid-cols-[180px_1fr] gap-4 lg:gap-12 items-baseline">
                <div className="flex items-center gap-3 lg:block lg:pt-1">
                  ...
                  <span className="font-black leading-none lg:block lg:mt-2" style={{ fontSize: "clamp(2.5rem, 4.5vw, 4rem)", color: "var(--lavender)", opacity: 0.95 }}>
                    03
                  </span>
                </div>
                <div className="max-w-2xl">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-start">
                <div className="lg:col-span-7 lg:pt-2 space-y-10 order-2 lg:order-1">
                  ...
                  <div className="pt-8 border-t border-foreground/10">
                    ...
                    <div className="grid grid-cols-3 gap-x-4 gap-y-5">
                      {interactions.items.map((it, i) => {
                        const colors = ["var(--lavender)", "var(--accent)", "var(--gold)"];
                        return (
                          <div key={i}>
                            <p className="font-mono font-bold leading-none mb-2" style={{ fontSize: "clamp(1.1rem, 1.6vw, 1.4rem)", color: colors[i % colors.length] }}>
                              {it.glyph}
                            </p>
                            <p className="text-[11px] uppercase tracking-[0.18em] text-foreground/55 leading-snug">
                              {it.label}
                ...
                <figure className="lg:col-span-5 mx-auto lg:mx-0 w-full max-w-[400px] lg:max-w-none lg:w-full order-1 lg:order-2">
                  <div className="relative rounded-2xl overflow-hidden border border-border/60 shadow-[0_32px_80px_-8px_rgba(0,0,0,0.6)]">
                    <Image
                      src={producerImage}
                      alt={`${projetTitre} — ${tProjet(`${SLUG}.page.figCaption.producer`)} FR`}
                      width={1080}
                      height={1920}
                      className="block w-full h-auto"
                      sizes="(max-width: 1024px) 90vw, 480px"
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-4 sm:gap-6 max-w-2xl">
                      <div className="relative rounded-xl overflow-hidden border border-border/60">
                        ...
                        />
                        <span className="absolute top-2 left-2 text-[9px] font-mono uppercase tracking-[0.3em] px-1.5 py-0.5 rounded bg-black/70 text-foreground/80">FR</span>
                      </div>
                      <div className="relative rounded-xl overflow-hidden border border-border/60">
                        <Image
                          src={producerImageEn}
                          ...
                          sizes="(max-width: 768px) 45vw, 280px"
                        />
                        <span className="absolute top-2 left-2 text-[9px] font-mono uppercase tracking-[0.3em] px-1.5 py-0.5 rounded bg-black/70 text-foreground/80">EN</span>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <Reveal key={i} delay={0.04} y={12} duration={0.45}>
                    <article className="group grid grid-cols-1 md:grid-cols-[1fr_2fr] gap-6 md:gap-12 py-9 border-t border-foreground/10">
                      <div className="space-y-3">
                        <span className="block font-black leading-none" style={{ fontSize: "clamp(2rem, 3vw, 2.75rem)", color: accent, opacity: 0.85 }}>
                          {num}
                        </span>
            ...
            <Reveal>
              <header className="grid grid-cols-1 lg:grid-cols-[180px_1fr] gap-4 lg:gap-12 items-baseline">
                <div className="flex items-center gap-3 lg:block lg:pt-1">
                  ...
                  <span className="font-black leading-none lg:block lg:mt-2" style={{ fontSize: "clamp(2.5rem, 4.5vw, 4rem)", color: "var(--mauve)", opacity: 0.95 }}>
                    06
                  </span>
                </div>
                <div className="max-w-2xl">
